    ...
    <div style={banner}>
      <StyledBannerDiv>
        <StyledLeftButton onClick={() => setIndex(bannerIndex - 1)}>
          <img src={evoButtonL} alt="Bal" width={"60px"} />
        </StyledLeftButton>
        <div style={{ width: "5%", height: "400px" }}></div>
        <Card
          title={product[[(amountOfProd + bannerIndex) % amountOfProd]].title}
          imageUri={
            product[[(amountOfProd + bannerIndex) % amountOfProd]].imageUri
          }
        ...
        <CardBig
          title={
            product[[(amountOfProd + bannerIndex + 1) % amountOfProd]].title
          }
          imageUri={
            product[[(amountOfProd + bannerIndex + 1) % amountOfProd]].imageUri
          }
          body={product[[(amountOfProd + bannerIndex + 1) % amountOfProd]].body}
          price={
            product[[(amountOfProd + bannerIndex + 1) % amountOfProd]].price
          }
        />
        <div style={{ width: "10%", height: "400px" }}></div>
        <Card
          title={
            product[[(amountOfProd + bannerIndex + 2) % amountOfProd]].title
          }
          imageUri={
            product[[(amountOfProd + bannerIndex + 2) % amountOfProd]].imageUri
          }
          body={product[[(amountOfProd + bannerIndex + 2) % amountOfProd]].body}
          price={
            product[[(amountOfProd + bannerIndex + 2) % amountOfProd]].price
          }
        />
        <div style={{ width: "5%", height: "400px" }}></div>
        <StyledRightButton onClick={() => setIndex(bannerIndex + 1)}>
          <img src={evoButton} alt="Jobb" width={"60px"} />
        </StyledRightButton>
      </StyledBannerDiv>
    </div>
  );
